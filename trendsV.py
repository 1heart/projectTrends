...
def tweet_location(tweet):
    ...
    # return latitude(tweet[2], tweet[3]), longitude(tweet[2], tweet[3])
...
def extract_words(text):
    ...
    word = ''
    for _ in text:
        if _ in ascii_letters:  #adding selected letters to a word
            word += _
        elif word != '':        #then adding the word to the list
            words += [word]
            word = ''
    if word != '':      #adds the final word to the list
        words += [word]
    return words
...
def get_word_sentiment(word):
    ...
    False
    """
    # Learn more: http://docs.python.org/3/library/stdtypes.html#dict.get_items
...
def analyze_tweet_sentiment(tweet):
    ...
    sentiment = 0
    counter = 0
    for _ in tweet_words(tweet):
        if has_sentiment(get_word_sentiment(str(_))):
            sentiment += sentiment_value(get_word_sentiment(str(_)))
            counter += 1
    if counter == 0:
        return make_sentiment(None)
    return make_sentiment(sentiment / counter)
...
def get_all_latitudes(polygon):
    """
    """
    latitudes = []
    for _ in polygon[:-1]:
        latitudes += [latitude(_)]
    return latitudes

def get_all_longitudes(polygon):
    longitudes = []
    for _ in polygon[:-1]:
        longitudes += [longitude(_)]
    return longitudes
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    latitudes, longitudes, area = get_all_latitudes(polygon), get_all_longitudes(polygon), 0
    for _ in range(len(latitudes)):
        area += latitudes[_ - 1] * longitudes[_] - longitudes[_ - 1] * latitudes[_]
    area = abs(area / 2)
    if area == 0:
        return [latitudes[0], longitudes[0], area]
    return [sum(latitudes) / len(latitudes), sum(longitudes) / len(longitudes), area]
...
def find_state_center(polygons):
    ...
    for _ in polygons:
        x_list, y_list, area_list = x_list + [find_centroid(_)[0] * find_centroid(_)[2]], y_list + [find_centroid(_)[1] * find_centroid(_)[2]], area_list + [find_centroid(_)[2]]
    return make_position(sum(x_list) / sum(area_list), sum(y_list) / sum(area_list))
...
def group_tweets_by_state(tweets):
    ...
    "*** YOUR CODE HERE ***"
    keys_and_values = make_database()
    for key in get_keys(us_states):
        keys_and_values = add_value(keys_and_values, key, find_state_center(get_value_from_key(us_states, key)))
    for tweet in tweets:
        distance = 9001
        current_state = 'Not'
        for key in get_keys(keys_and_values):
            if geo_distance(get_value_from_key(keys_and_values, key), tweet_location(tweet)) < distance:
                distance = geo_distance(get_value_from_key(keys_and_values, key), tweet_location(tweet))
                current_state = key
        if current_state not in get_keys(tweets_by_state):
            tweets_by_state = add_value(tweets_by_state, current_state, [tweet])
        else:
            old_value = get_value_from_key(tweets_by_state, current_state)
            tweets_by_state = add_value(tweets_by_state, current_state, old_value + [tweet])
...
def average_sentiments(tweets_by_state):
    ...
    for state in get_keys(tweets_by_state):
        list_of_analyzations = []
        for tweet in get_value_from_key(tweets_by_state, state):
            if analyze_tweet_sentiment(tweet) != None:
                list_of_analyzations += [analyze_tweet_sentiment(tweet)]
            print(list_of_analyzations)
        if len(list_of_analyzations) != 0:
            averaged_state_sentiments = add_value(averaged_state_sentiments, state, sum(list_of_analyzations) / len(list_of_analyzations))
        print('lmao', get_keys(averaged_state_sentiments))
