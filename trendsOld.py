...
def analyze_tweet_sentiment(tweet):
    ...
    total, n = 0, 0
    text = extract_words(tweet_string(tweet))
    for i in range(len(text)):
    	current_word = get_word_sentiment(text[i])
    	if has_sentiment(current_word) != False:
    		total, n = total + sentiment_value(current_word), n+1
    if n != 0:
    	average = make_sentiment(total/n)
    "*** YOUR CODE HERE ***"
    return average
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    x = [latitude(elem) for elem in polygon]
    y = [longitude(elem) for elem in polygon]
    xG, yG = x, y ##keep these values when you go into centroidCoord
    def area():
        summation = 0
        for i in range(0, len(x)-1):
            summation += x[i] * y[i+1] - x[i+1] * y[i]
        return (summation/2)
    if area() == 0:
        return [x[0],y[0],0]
    def centroidCoord(x,y):
        summation = 0
        for i in range(0, len(x)-1):
            summation += (x[i] + x[i+1]) * (xG[i] * yG[i+1] - xG[i+1] * yG[i])
        return summation/(6*area())
    return [centroidCoord(x,y),centroidCoord(y,x),abs(area())]
...
def find_state_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    def findCoord(x_or_y): #if x, 0; if y, 1
    	sumNum, sumDen = 0, 0
    	for i in range(len(polygons)):
    		c = find_centroid(polygons[i])
    		sumNum, sumDen = sumNum + c[x_or_y] * c[2], sumDen + c[2]
    	return sumNum/sumDen
    return make_position(findCoord(0), findCoord(1))
...
def group_tweets_by_state(tweets):
    ...
    "*** YOUR CODE HERE ***"
    stateCenterDatabase = make_database() #the for-loop below creates the database of state centers
    for i in range(len(get_keys(us_states))):
        stateCenter = find_state_center(get_value_from_key(us_states, get_keys(us_states)[i]))
        stateCenter = make_position(stateCenter[0], stateCenter[1])
        stateCenterDatabase = add_value(stateCenterDatabase, get_keys(us_states)[i], stateCenter)
    def findClosestState(tweet): ##find closest state using generated database
        closestState, closestDistance = 'CA', 100000
        for i in range(len(get_keys(stateCenterDatabase))):
            theState = get_keys(stateCenterDatabase)[i]
            theLocation = get_value_from_key(stateCenterDatabase, theState)
            theLocation = make_position(theLocation[0],theLocation[1])
            dist_from_state = geo_distance(tweet_location(tweet), theLocation)
            if dist_from_state < closestDistance:
                closestState, closestDistance = get_keys(us_states)[i], dist_from_state
        return closestState
    for i in range(len(tweets)):
        theClosestState = findClosestState(tweets[i])
        previousValue = [] #the following code is to prevent overwriting the values
        if theClosestState in get_keys(tweets_by_state):
            previousValue = get_value_from_key(tweets_by_state, theClosestState)
        tweets_by_state = add_value(tweets_by_state, theClosestState, previousValue + [tweets[i]]) #this add_value function sucks, needs the previous code to stop overwriting
...
def average_sentiments(tweets_by_state):
    ...
    "*** YOUR CODE HERE ***"
    for state in range(len(get_keys(tweets_by_state))): #operating on each state
        totalSentiment, n = 0, 0
        for tweet in get_value_from_key(tweets_by_state, get_keys(tweets_by_state)[state]): #operating on each tweet in the state
            theTweetSentiment = analyze_tweet_sentiment(tweet)
            if has_sentiment(theTweetSentiment):
                totalSentiment, n = totalSentiment + sentiment_value(theTweetSentiment), n+1
        if n != 0: #if there are tweets with sentiments
            averaged_state_sentiments = add_value(averaged_state_sentiments, get_keys(tweets_by_state)[state], totalSentiment/n)
