...
def tweet_location(tweet):
    ...
    "*** YOUR CODE HERE ***"
    return tweet[2:] #is this a DAT violation?
...
def make_tweet_fn(text, time, lat, lon):
    ...
    def tweet(msg):
        ...
        if msg == 'lon':
            return lon
        def make_position(tweet1, tweet2):
...
def tweet_location_fn(tweet):
    ...
    #make_position takes in 2 functions(tweets)
### === +++ ABSTRACTION BARRIER +++ === ###
...
def extract_words(text):
    ...
    for let in text:
        if let in letters: #if valid letter
            new_text += let
        if let not in letters:
            new_text += " " #add a space to avoid 'elsenot'
    return new_text.split()  # You may wish to change this line.

def make_sentiment(value): #DOMAIN: -1 to 1 OR None RANGE: value
...
def sentiment_value(s):
    ...
    "*** YOUR CODE HERE ***"
    return s
def get_word_sentiment(word): #DOMAIN: word RANGE: value
...
def find_centroid(polygon):
    ...
    def area():
        ...
        for i in range(len(polygon)-1):
            x = latitude(polygon[i]) #returns latitude from make_position
            y = longitude(polygon[i]) #returns longitude from make_position
            x2 = latitude(polygon[i+1])
            y2 = longitude(polygon[i+1])
            total += (x*y2) - (x2*y)
        return 0.5 * total
    def centroid_x(): #finds x according to the wiki
        total = 0
        for i in range(len(polygon)-1):
            x = latitude(polygon[i])
            y = longitude(polygon[i])
            x2 = latitude(polygon[i+1])
            y2 = longitude(polygon[i+1])
            total += (x + x2) * ((x*y2) - (x2*y))
        return (1/(6*area())) * total
    def centroid_y(): #wiki for y
        total = 0
        for i in range(len(polygon)-1):
            x = latitude(polygon[i])
            y = longitude(polygon[i])
...
def find_state_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    #DOMAIN: list of polygons
    #RANGE: make_position, the state
    def cx():
        num = 0
        den = 0
        for i in polygons:
            num += find_centroid(i)[0] * find_centroid(i)[2]
            den += find_centroid(i)[2]
        return num/den
    def cy():
        num = 0
        den = 0
        for i in polygons:
            num += find_centroid(i)[1] * find_centroid(i)[2]
            den += find_centroid(i)[2]
        return num/den
...
def group_tweets_by_state(tweets):
    ...
    for i in get_keys(us_states):
        state_centers = add_value(state_centers, i, find_state_center(i))
    for tweet in tweets:
        for i in state_centers:
            if distance > geo_distance(tweet_location(tweet), i):
                distance = geo_distance(tweet_location(tweet), i)
